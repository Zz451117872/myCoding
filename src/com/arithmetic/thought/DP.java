...
public class DP {		
	/*
	 * 钢条切割问题：有一根很长的钢条，不同尺寸的钢条价格不一样，如何切目标钢条使收益最大化
	 * p:不同尺寸钢条对应的价格
	 * n:需要切割钢条的长度
	 */
	//至底向上 方式 
	...
	public static int maxLoadValue(int space,int[] goods,int[] value)
	{
		...
		for(int i=0; i<row; i++)	//当x为0 或者 y为0时，可以装入的价值都为0
		{
			dp[i][0] = 0;
		}
		...
		for(int i=1; i<= goods.length; i++)//至底向上 递推， i表示前几个物品，ｊ则表示背包的容量
		{
			for(int j=1; j<=space ;j++)
			{
				dp[i][j] = 0;
				int index = i;
				while(index >= 1 && j < goods[index -1] )//这里的判断，意思是当前的空间不足以装下当前物品时，可以试装更小的物品
				{
					index--;
				}
				if(j >= goods[index -1])
	...
	public static int loadMaxSpace(int[] goods,int space)
	{
		...
		int[][] dp = new int[row][column];
		for(int i=0; i<row; i++)	//当 y 为0 或者 x为0时，可装入空间都为0
		{
			dp[i][0] = 0;
		}
		for(int i=0; i<column; i++)
	...
	public static boolean down_up_coinValuePlay(int[] value)
	{
		...
		//dp[i] 表示从i到end 可获取的最大价值
		dp[value.length] = 0;		//当 i==end时，没有东西拿了，所有为0
		dp[value.length - 1] = value[value.length-1];		//当 i==end-1时，只有最后1个硬币可以拿，所以直接拿
		dp[value.length - 2] = value[value.length -2]+value[value.length-1];//当 i==end-2时，有2个硬币可以拿，所以直接全部拿
		dp[value.length -3] = value[value.length -3]+value[value.length-2];//当 i==end-3时，有3个硬币可以拿，价值最大化，所有拿2个
