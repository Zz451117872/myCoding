...
public class DP2 {
	...
	/**
	 * 数塔问题 至底向上 
	 * 从指定的坐标点往数塔底层遍历所得的最大值
	 * @param tower :传入的数塔，一个二维数组
	 * @param x ：需要检查的坐标点
	 * @param y ：需要检查的坐标点
	 * @return
	 */
	...
	{		
		...
		if(x == tower.length-1)
		{
			...
			//如果不是最后一行，则计算能获取的最大值
			dp[x][y] = Math.max( 
					digitTowerByRecursion_aux(dp,tower,x+1,y), 
	...
	private static int partitionPalindromicAux(int[] dp, String palindromic, int end) 
	{
		...
		{						
			...
			dp[end] = Math.min(
					partitionPalindromicAux( dp , palindromic , end-longest ) + 1, 
					partitionPalindromicAux( dp, palindromic , end-1 ) + 1 );
		}else{					
			//如果待切割字符串末尾没有回文串，则切掉末尾1个字符
	...
	/**
	 * 分割回文数 纯递归
	 * 与 动态规划的方式是一样的，但动态规划缓存了子问题的，提高了效率
	 * @param s
	 * @param start
	 * @param end
	 * @return
	...
	public static int partitionPalindromicByRecursion(String s,int start, int end)
	{
		...
		if(isPalindromic(s.substring(start,end+1)))
		{
			...
			{
				return Math.min( 
						partitionPalindromicByRecursion(s,start,end-longest) + 1, 
	...
	public static int down_up_partitionPalindromic(String palindromic)
	{
		...
		for(int i=2; i<=palindromic.length(); i++)
		{
			...
			int longest = getLongestTailPalindromic(target);
			if(longest >0){	
				//如果待切字符串末尾有回文串，则试着切掉回文串 或者 只切1个字符，看谁小
				dp[i] = Math.min(dp[i-longest] + 1, dp[i-1] +1);			
			}else{				
				//如果待切字符串末尾没有回文串，则只切1个字符
				dp[i] = dp[i-1]+1;
			}
		}		
	...
	{
		int result = 0; 
		//从字符串的末尾一直往前切，当切掉的子串是回文数时，更新结果值
