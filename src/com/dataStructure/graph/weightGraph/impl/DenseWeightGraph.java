...
//稠密图：实现方式为 邻接矩阵
public class DenseWeightGraph<T extends Comparable<T>> implements IWeightGraph<T>{
	...
	public DenseWeightGraph(int vertex,boolean directed)
	{
		this.vertex = vertex;
		this.edge = 0;
	...
	class AdjIterator{
		private DenseWeightGraph<T> G;
		private int index;
		private int vertex;
		
		public AdjIterator(int vertex ,DenseWeightGraph<T> G)
		{
			this.G = G;
			this.vertex = vertex;
			this.index = 0;
